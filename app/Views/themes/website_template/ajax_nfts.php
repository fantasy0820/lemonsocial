    ...
    curl_setopt($cURLConnection, CURLOPT_URL, 'https://min-api.cryptocompare.com/data/pricemulti?fsyms=ETH,MATIC&tsyms=USD');
    curl_setopt($cURLConnection, CURLOPT_RETURNTRANSFER, true);
    
    $phoneList = curl_exec($cURLConnection);
    curl_close($cURLConnection);
    
    $r = json_decode($phoneList);
        ...
        <div class="card nft-items nft-primary rounded-7 border-0 overflow-hidden mb-1 p-4" startdate="<?php echo esc($nft->start_date); ?>" enddate="<?php echo esc($nft->end_date); ?>"> 
            ...
            <div class="card-body content position-relative mt-3">
                ...
                <div class="d-flex justify-content-between mt-2">
                    <small class="rate fw-semi-bold text-primary"><?php echo number_format($nft->min_price, 6, '.', ',').' '.$chain; ?></small>
                    <br>
                    <small class="rate fw-semi-bold text-primary"><?php echo number_format($value * $nft->min_price, 2, '.', ',').' USD ' ?></small>
                </div>
            </div>
        </div>
    </div>
