...
class User_nfts extends BaseController
{
  ...
  */
  ...
  public function add_collection()
  {
    ...
    $data['blockchain'] = $this->db->table('blockchain_network')->select('id, network_name')->where(['status' => 1])->get()->getResult();
  ...
  public function create_new_nft_action()
  {
    ...
    if ($this->validation->withRequest($this->request)->run()) {
      ...
      $lock = $this->request->getFile('lockable', FILTER_SANITIZE_STRING);

      $file_ext = pathinfo($_FILES["lockable"]["name"], PATHINFO_EXTENSION);
      ...
      if ($lock->getSize() != 0) {
        ...
        if ($file_ext == 'mp4' || $file_ext == 'webm') {
          ...
          if ($file->isValid() && !$file->hasMoved()) {
            ...
            $lock->move($path1, $newName);
            $lockimage = '/' . $path1 . '/' . $newName;
          } else {
          ...
          if ($lock->isValid() && !$lock->hasMoved()) {
            $newName = $lock->getRandomName();
            $lock->move($path2, $newName);
            $lockimage = '/' . $path2 . '/' . $newName;
          } else {
          ...
          if ($lock->isValid() && !$lock->hasMoved()) {
            $newName = $lock->getRandomName();
            $lock->move($savepath, $newName);
            $lockimage = '/' . $savepath . '/' . $newName;
          } else {
      ...
      $endpoint_url = "https://api.nft.storage/upload";

      // Set your NFT.storage API key
      $api_key = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJkaWQ6ZXRocjoweDViOEJCMDFjNmM1MDE4QjQ5MDk5ZDk5MjdCMmQ4MjYxODNhMkI3QjEiLCJpc3MiOiJuZnQtc3RvcmFnZSIsImlhdCI6MTY3MDI2ODA1NTE5NSwibmFtZSI6IkxlbW9uc29jaWFsIn0.u_mJ12qeJtt8DGlY39FXc4H5TtoHFHoCeEGlJ7Vf8i4';

      // Set the file path of the NFT image file to upload
      $file_path = base_url() . $returnIdInfo->file;

      // Set the metadata for the NFT
      $metadata = array(
        "name" => $this->request->getVar('item_name', FILTER_SANITIZE_STRING),
        "description" => $this->request->getVar('description', FILTER_SANITIZE_STRING),
        "image" => base_url() . $returnIdInfo->file
      );
      $ipfs = '';
      ...
      if (curl_errno($curl)) {
        ...
        die(json_encode(['status' => 'err', 'msg' => 'An error occured when pushing to Ipfs, Please retry and if this persist Please Notify the administrator']));
        ...
        $ipfs = "https://ipfs.io/ipfs/" . $json->value->cid;
      }
  ...
  public function new_nft_update()
  {
    ...
    $net = $this->request->getPost('net', FILTER_SANITIZE_STRING);

    $trx_hash = $this->request->getPost('trx_hash', FILTER_SANITIZE_STRING);
    if ($type == 'free') {
      $owner_wallet = $this->session->userdata('wallet_address');
      $data['type'] = 'free';
      $data = array(
        'token_id' => $token_id,
