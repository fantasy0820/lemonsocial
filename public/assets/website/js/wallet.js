...
let check_matic = document.getElementById("nft_net");

let matic_address = "0x2Bce86a9DfE21AFc72A1FbFb25715f5A222BaCDD"; //'0x81a8Eca452dabB2CDF81405a981D3Faf3d7D5e43';//'0x2Bce86a9DfE21AFc72A1FbFb25715f5A222BaCDD';
...
$("body").on("submit", "#createNftform", async function (event) {
  ...
  if (networkVerifyMint(chainId) == false) {
    $(".mint-submit").html(
  ...
  $.ajax({
    ...
    success: function (res) {
      ...
      if (res.contractAddress != "" && res.img_path != "") {
        // Replace YOUR_API_KEY with your NFT storage API key
        const apiKey =
          "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJkaWQ6ZXRocjoweDViOEJCMDFjNmM1MDE4QjQ5MDk5ZDk5MjdCMmQ4MjYxODNhMkI3QjEiLCJpc3MiOiJuZnQtc3RvcmFnZSIsImlhdCI6MTY3MDI2ODA1NTE5NSwibmFtZSI6IkxlbW9uc29jaWFsIn0.u_mJ12qeJtt8DGlY39FXc4H5TtoHFHoCeEGlJ7Vf8i4";
        ...
        // Send the metadata to NFT storage using an Ajax request
        $.ajax({
          type: "POST",
          ...
          success: function (data) {
            // Get the token Uri from the response data
            const tokenUri = "https://ipfs.io/ipfs/" + data.value.cid;
            console.log(tokenUri);

            if (document.getElementById("free").value == 2) {
              freeMintToken(res.contractAddress, tokenUri, res.nft_id);
            } else {
              let t = "eth";
              if (chainId == 5) {
                t = "goerli";
              ...
              let c = res.contractAddress;
              let m = document.getElementById("free").value;
              if (m == 3) {
                c = "0x2Bce86a9DfE21AFc72A1FbFb25715f5A222BaCDD";
                t = "matic";
              }

              mintToken(c, tokenUri, res.nft_id, "normal", t);
            }
            // Use the tokenUri to mint the NFT on your smart contract
            // ...
          },
...
async function freeMintToken(contractAddress = "", tokenURI = "", nftId = "") {
  let postData = {};
  postData[csrf_token] = get_csrf_hash;
  postData["owner_wallet"] = " ";
  postData["contract_address"] = contractAddress;
  postData["trx_hash"] = " ";
  postData["token_id"] = 1 + parseInt(nftId, 16);
  postData["nftId"] = nftId;
  postData["type"] = "free";

  console.log(postData["token_id"]);
...
$("body").on("submit", "#list_for_sale_form", async function (event) {
  ...
  let g = 1000000;
  if (check_matic && check_matic == "matic") {
...
$(document).on("click", "#buyNowubmit", async () => {
  ...
  $.ajax({
    ...
    success: function (res) {
      if (res.status == "success") {
        var t = document.getElementById("type_nft").value;
        var to = document.getElementById("sellerWallet").value;
        console.log("seller wallet  " + to);

        if (t == "free") {
          SendETH(res.contract, res.token_id, res.fee, res.price, nftId, to);
          // SendETH(contractAddress, tokenID, fees, sellPrice, nft_id, to)
...
async function SendETH(contractAddress, tokenID, fees, sellPrice, nft_id, to) {
  try {
    let gotAccount = await ethereum.request({ method: "eth_accounts" });
    let account = gotAccount.length;

    console.log(gotAccount[0]);
    ...
    // const tx = {
    //     from: gotAccount[0],
    //     to: to, //"0x611E72c39419168FfF07F068E76a077588225798",
    //     value: ethers.utils.parseEther(sellPrice),
    //     nonce: await provider.getTransactionCount(gotAccount[0], "latest"),
    ...
    const params = [
      {
        from: gotAccount[0],
        to: to,
        value: ethers.utils.parseUnits(sellPrice, "ether").toHexString(),
      },
    ];
    ...
    const transactionHash = await provider
      ...
      .then((transactionHash) => {
        toasterMessage("success", "transaction confirmed, please wait...");

        let url = base_url + "/nfts/fix-buy-complete";
        let inputdata = {};
        inputdata["nft_id"] = nft_id;
        inputdata["token_id"] = tokenID;
        inputdata["trx_info"] = JSON.stringify(transactionHash);
        ...
        $.ajax({
          ...
          success: function (result) {
            if (result.status == "success") {
              ...
              var check = confirm(
                "Press Okay to mint this NFT or cancel to Leave as Free minting"
              );

              if (check) {
                var nft_part = document.getElementById("nft_part").value;

                mintToken(contractAddress, nft_part, nft_id);
              } else {
                window.location.href = base_url + "/user/dashboard";
              }
            } else {
      ...
      .catch((error) => {
        toasterMessage(
          "error",
          "something went wrong when pushing to blockchain"
        );
        $(".buy-now-submit").html(
          '<button type="button" id="buyNowubmit" nft_id="' +
            nft_id +
    ...
    //   var sellResult =   await signer.sendTransaction(tx).then((transaction) => {
...
//SendETH('0.0003', '0x19647Ab1c0d9D23E3698FC769C26509657Bc1Afb');
...
async function fixBuyConfirm(
  ...
  try {
    ...
    let sellResult = await contract.sellNFT(
      ...
      sellerGets,
      marketOwnerGets,
      { value: ethers.utils.parseUnits(sellPrice, "ether"), gasLimit: 200000 }
    );
...
function networkVerifyMint(chainId) {
  let m = document.getElementById("free").value;

  if (m == 3 && chainId != 137) {
    toasterMessage("warning", "Please select Polygon network!");
    return false;
  } else if ((m == 3 && chainId == 137) || chainId == 1) {
    return true;
  } else {
...
function networkVerify(chainId) {
  let m = document.getElementById("nft_net");

  if (m) {
    m = m.value;
    if (m == "matic" && chainId != 137) {
      toasterMessage("warning", "Please select Polygon network!");
      return false;
    } else if (m == "matic" && chainId == 137) {
      return true;
    }
  }
...
$(document).on("click", "#makeOfferBtn", async function () {
  ...
  if (check_matic && check_matic.value == "matic") {
    marketContractAddress = matic_address;
  }
  //network check
  const _chainId = await ethereum.request({
    method: "eth_chainId",
